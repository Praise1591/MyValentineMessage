...
function LandingPage() {
  ...
  // Auto-refresh deliveries every 10 seconds to show updated locations
  useEffect(() => {
  ...
  // FIXED: Refresh function to load latest deliveries
  const refreshDeliveries = () => {
    try {
      const deliveries = JSON.parse(localStorage.getItem("cycle_deliveries") || "[]");
      console.log("📦 All deliveries in localStorage:", deliveries);
      
      // Filter by email - case insensitive
      const userDeliveries = deliveries.filter(d => {
        const emailMatch = d.customerEmail?.toLowerCase() === customerEmail.toLowerCase();
        const nameMatch = d.customerName?.toLowerCase() === customerEmail.split('@')[0].toLowerCase();
      ...
      // If no deliveries found, show a helpful message
      if (userDeliveries.length === 0 && deliveries.length > 0) {
        console.log("No deliveries found for this email. Available customer emails:", 
          deliveries.map(d => d.customerEmail).filter(Boolean));
      }
  ...
  // FIXED: Handle Track Package with better error handling
  const handleTrackPackage = () => {
    ...
    try {
      ...
      const delivery = deliveries.find(d => 
        d.trackingId?.toUpperCase() === trackingId.trim().toUpperCase()
      );
  ...
  // FIXED: SECURE ADMIN ACCESS - Hidden in Customer Portal
  const handleCustomerLogin = () => {
    ...
    setTimeout(() => {
      try {
        const deliveries = JSON.parse(localStorage.getItem("cycle_deliveries") || "[]");
        console.log("🔍 Searching for deliveries with email:", customerEmail);
        ...
        if (userDeliveries.length > 0) {
          console.log("📦 Deliveries found:", userDeliveries);
        }
        ...
        if (userDeliveries.length === 0) {
          // Suggest checking email or creating a test delivery
          toast.error(`No deliveries found for "${customerEmail}". Please check with the sender.`);
          
          // If there are deliveries in the system, show a hint
          if (deliveries.length > 0) {
            const availableEmails = [...new Set(deliveries.map(d => d.customerEmail).filter(Boolean))];
            console.log("💡 Available customer emails in system:", availableEmails);
            
            // Check if the email exists in the system but with different case
            const caseInsensitiveMatch = deliveries.some(d => 
              d.customerEmail?.toLowerCase() === customerEmail.toLowerCase()
            );
            
            if (caseInsensitiveMatch) {
              toast.info("💡 Tip: Make sure you're using the exact email used when creating the delivery.");
            }
          }
        } else {
